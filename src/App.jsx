...
function AlertsTable({ rows }) {
  if (!rows.length) {
    return <div className="card">No alerts found</div>;
  }
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id}>
              ...
              <td>{r.type}</td>
              <td>{r.title}</td>
              <td>{JSON.stringify(r.payload)}</td>
              <td>{r.created_at}</td>
            </tr>
          ))}
        </tbody>
...
function ActionPanel({ tab, row, onSaved }) {
  ...
  if (!row) {
    return (
      <div className="card">
        Select a row to update status, owner, notes, or send notification.
      </div>
    );
  }
      ...
      <div className="actions" style={{ marginTop: 12 }}>
        ...
        <div>
          ...
          <input
            ...
            placeholder="Reception / HR / Tech / Admin"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Follow-up note / remarks"
          />
        </div>
...
export default function App() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [leads, admissions, appointments, support, faculty, alerts] =
        await Promise.all([
          api.get("/admin/leads"),
      ...
      setError(
        "Could not load backend data. Check Render backend and wait if it is waking up."
      );
    }
  };
        ...
        <div className="row">
          <div>
            ...
            <div className="notice">
              Leads, admissions, appointments, support, faculty, alerts, action panel
            </div>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button className="orange" onClick={loadAll}>
              Refresh All
            </button>
          </div>
        </div>
